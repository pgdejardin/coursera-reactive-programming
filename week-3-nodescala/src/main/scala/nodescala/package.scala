...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      def compose(fS: List[Future[T]]): List[T] = {
        if (fs.isEmpty) List[T]()
        else Await.result(fs.head, 0 nanos) :: compose(fs.tail)
      }
      var count = 0
      ...
      fs foreach { f =>
        f onComplete {
          ...
          case Success(s) =>
            count += 1
            if (count == fs.length) p.success(compose(fs))
        }
      }
      p.future
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def now: T = {
      try {
        ...
        case e: TimeoutException => throw new NoSuchElementException("No result yet.")
      }
    }
